...
public class MediaPlayerCtrl : MonoBehaviour
{
    ...
    public Text exceptionTEXT; // Is the load function called

    public Text stateTEXT; // Current state
    ...
    void Start()
    {
        ...
        //Handheld.PlayFullScreenMovie("EasyMovieTexture.mp4", Color.black, FullScreenMovieControlMode.CancelOnInput);
	...
	private unsafe bool Call_Load(string strFileName, int iSeek)
	{
		...
		if( strFileName.Contains("://") == false)
		{
            ...
            //strFileName = "storage/emulated/0/videos" + "/" + strFileName;

            Debug.Log(strFileName);
            this.filepath.text = this.m_strFileName;
		}
	...
	Thread loader = new Thread ( () => {
		...
		if (ffmpeg.avformat_open_input(&ppFomatContext, strFileName, null, null) != 0)
		{
            ...
            this.exceptionTEXT.text = "Exception: Could not open file";
		...
		{
			m_CurrentState = MEDIAPLAYER_STATE.ERROR;
            this.exceptionTEXT.text = "Exception: Could not find stream info";
...
void LoadVideoPart2 ()
{
		...
		{
			m_CurrentState = MEDIAPLAYER_STATE.ERROR;
            this.exceptionTEXT.text = "Exception: Could not found video stream";
		...
		{
			m_CurrentState = MEDIAPLAYER_STATE.ERROR;
            this.exceptionTEXT.text = "Exception: Could not initialize the conversion context";
		...
		{
			m_CurrentState = MEDIAPLAYER_STATE.ERROR;
            this.exceptionTEXT.text = "Exception: Unsupprted codec";
		...
		{
			m_CurrentState = MEDIAPLAYER_STATE.ERROR;
            this.exceptionTEXT.text = "Exception: Could not open codec";
		...
		if( m_CurrentState != MEDIAPLAYER_STATE.PAUSED)
		{
			if (pPacket != null) 
            {
				if (pPacket->stream_index == iStreamIndex) {
					...
					if (size < 0) {
                        this.exceptionTEXT.text = "Exception: Error while decoding frame";
				...
				do{
                    ...
                    if ( ret < 0)
					{
                        ...
                        {
                        this.exceptionTEXT.text = "Exception: Could not read frame";
